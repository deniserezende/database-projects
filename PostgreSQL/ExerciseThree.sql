...
CREATE TABLE "LIBRARY".log_book_author_changes (
    log_id SERIAL NOT NULL,
    book_id INTEGER, -- NOT NULL, should be not null but BUG.
...
BEGIN
	...
	FOR author IN (SELECT DISTINCT author_name FROM "LIBRARY".book_authors) LOOP
		-- Selecting one by one all other authors different from the first one from book_authors
    	FOR distinct_author IN (SELECT DISTINCT author_name FROM "LIBRARY".book_authors WHERE author_name <> author.author_name) LOOP 
			-- Debug
			...
			IF names_should_be_normalized(author.author_name, distinct_author.author_name) THEN
				...
				SELECT COUNT(*) INTO amoutnOne FROM "LIBRARY".book_authors WHERE author_name = author.author_name;
				SELECT COUNT(*) INTO amoutnTwo FROM "LIBRARY".book_authors WHERE author_name = distinct_author.author_name;
				...
				SELECT book_id INTO book_id_temp FROM "LIBRARY".book_authors WHERE author_name = less_frequent;
				-- BUG: debug!!!
				RAISE NOTICE 'book_id_temp %', book_id_temp;	
				INSERT INTO "LIBRARY".log_book_author_changes (book_id, old_name, new_name)
...
SELECT * FROM "LIBRARY".log_book_author_changes;


-- DATA TO TEST EXERCISE THREE:
INSERT INTO "LIBRARY".publisher(publisher_name) VALUES('Publisher 1');
